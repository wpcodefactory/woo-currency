...
abstract class tableWcu {
	...
	/**
	 * Alias for this table, make sure that it is unique
	 */
	protected $_alias = '';
	...
	public function getTable($transform = false) {
		if($transform)
			return dbWcu::prepareQuery($this->_table);
		else
			return $this->_table;
	}
	...
	public function store($data, $method = 'INSERT', $where = '') {
		...
		if(dbWcu::query($query)) {
			...
			$this->_addError(WCU_TEST_MODE ? dbWcu::getError() : __('Database error. Please contact your developer.', WCU_LANG_CODE));
		return false;
	}
	 ...
	 * if string given - just return it without changes
				...
				if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
					...
					if($validate) {
						if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
							...
							if($errors = validatorWcu::_($objForValidation)) {
	...
	/**
	 * Add new fieldWcuWcu for children table (@see class field)
	...
	protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
		$this->_fields[$name] = toeCreateObjWcu('fieldWcu', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
			...
			switch($f->type) {
				case 'date':
					if($d[$key] == '0000-00-00' || empty($d[$key]))
						...
						$d[$key] = date(WCU_DATE_FORMAT, dbWcu::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
