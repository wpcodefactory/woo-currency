...
class validatorWcu {
    ...
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldWcu') {
            $value = $field;
            $field = new fieldWcuWcu('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorWcu', $v))
                    self::$v($field);
            }
        }
        if(method_exists('validatorWcu', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorWcu');
        foreach($all as $m) {
